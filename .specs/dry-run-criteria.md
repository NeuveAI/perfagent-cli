...
| 7 | Tool calls succeed (no `tool error`), Gemma calls `trace start` but stops without `trace stop` or `trace analyze` | **Workflow incomplete, not schema failure.** Gemma saw the `trace start` ack and decided the task was done. | Prefer `autoStop: true` in the first trace-start example; clarify in LOCAL_AGENT_SYSTEM_PROMPT. Not a P2 signal. |
| 8 | `tool error` text mentions browser/profile conflict, "already running", or chrome-devtools-mcp startup failure | **Infrastructure/lifecycle issue, not schema.** Zombie Chrome processes from prior runs holding the profile dir. | Run `pkill -f chrome-devtools-mcp/chrome-profile` and retry. Long-term: see task #46 (Chrome teardown/bootup). Not a P2 signal. |
