...
- **PRIMARY P1 candidate — structural REFLECT-injection probe.** harness-r2 P3 surfaced **0/240 non-abort ASSERTION_FAILED + 16/240 abort-category** (all gemini-react, all single-shot run-terminations for unrecoverable infrastructure conditions like CAPTCHA / WAF / anti-bot / HTTP2 protocol error; gemma-react 0/80 across all 80 traces). Models emit AF only for unrecoverable aborts, NEVER as a recoverable-error / retry signal — so the AF→REFLECT→PLAN_UPDATE retry-signal pathway is dead. Harness should detect **shape patterns** (NOT AF emission counts): SchemaError-loops (N consecutive identical-shape SchemaErrors on same stepId), arg-rejection-after-N-retries (the explicit-trigger pattern P3 tried to elicit). When detection fires → inject synthetic REFLECT directive into next observation, bypassing the dead retry-signal gate entirely. Adjacent to existing doom-loop detector in `tool-loop.ts` (3-identical-ACTION pattern). Canonical input fixture: `evals/traces/wave-harness-r2-plan-update/gemma-react__journey-1-car-configurator-bmw.ndjson` (R3 BMW shape persisted with explicit prompt teaching). Direct empirical evidence from harness-r2 P3 supports this is the only viable PLAN_UPDATE elicitation path.
