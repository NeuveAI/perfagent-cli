...
describe("PlanDecomposer frontier mode (structured output)", () => {
  it("decodes the structured plan from the AI SDK into >=4 steps", async () => {
    const plan = await runWithLayer(
      Effect.gen(function* () {
        const decomposer = yield* PlanDecomposer;
        return yield* decomposer.decompose(CATALOG_CHECKOUT_PROMPT, "frontier", decomposeContext);
      }),
      decomposerLayerFromModel(buildModelReturning(catalogCheckoutPlan)),
    );
  ...
  it("accepts the structured-output happy path (single JSON object)", async () => {
    ...
    const plan = await runWithLayer(
      Effect.gen(function* () {
        const decomposer = yield* PlanDecomposer;
        return yield* decomposer.decompose(DOCS_SEARCH_PROMPT, "frontier", decomposeContext);
      }),
  ...
  it("surfaces a DecomposeError when the model response is malformed JSON", async () => {
    const exit = await runExitWithLayer(
      Effect.gen(function* () {
        const decomposer = yield* PlanDecomposer;
        return yield* decomposer.decompose(FORM_WIZARD_PROMPT, "frontier", decomposeContext);
      }),
...
describe("PlanDecomposer no-API-key path (CRITICAL-1 regression)", () => {
  ...
  // the CLI actually wires up via layerCli) must build without the Gemini
  // API key. Template-mode and `none` callers never touch the frontier
  // planner, so requiring the key at layer-build time would break both the
  // TUI bootup and the gemma runner (which is Gemma-only and has no
  // business requiring a Gemini key).
  ...
  it("frontier mode without the key surfaces a DecomposeError (lazy key read fires on first planFrontier call)", async () => {
    const exit = await runExitWithLayer(
      Effect.gen(function* () {
        const decomposer = yield* PlanDecomposer;
        return yield* decomposer.decompose(CATALOG_CHECKOUT_PROMPT, "frontier", decomposeContext);
    ...
    if (Exit.isFailure(exit)) {
      ...
      if (Option.isSome(failure)) {
        ...
        expect(failure.value.cause).toContain("--planner template");
      }
    }
  });
  ...
  it("PlannerConfigError surfaces an actionable message", () => {
    ...
    expect(error.message).toContain("--planner template");
  });
});
