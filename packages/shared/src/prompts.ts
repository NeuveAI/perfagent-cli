...
export const buildLocalAgentSystemPrompt = (): string =>
  [
    ...
    "`action` is one of: insert | replace | remove | replace_step.",
    "`payload` is an AnalysisStep object.",
    "</plan_update_protocol>",
    "",
    "<reflect_trigger>",
    "After 2 consecutive ASSERTION_FAILED envelopes on the same stepId, the next observation contains a REFLECT directive — revise via PLAN_UPDATE or a different ACTION.",
    "</reflect_trigger>",
    "",
    "<completion_check>",
    "Emit RUN_COMPLETED status=passed only when every plan step is STEP_DONE and its expectedOutcome is verified against the latest <observation>. RUN_COMPLETED status=failed only after ASSERTION_FAILED on the unsatisfied step; to abort emit ASSERTION_FAILED with category=abort, then RUN_COMPLETED status=failed. Stop after RUN_COMPLETED — no further envelopes.",
    "</completion_check>",
    "",
    "<failure_categories>",
    ...
    "Closed `args.command` sets — interact: {navigate,click,type,fill,press_key,hover,drag,fill_form,upload_file,handle_dialog,wait_for,resize,new_tab,switch_tab,close_tab}; observe: {snapshot,screenshot,console,network,pages,evaluate}; trace: {start,stop,analyze,memory,lighthouse,emulate}.",
    ...
    "`evaluate` is observe.command, NOT interact. Envelope `_tag`s (THOUGHT/STEP_DONE/RUN_COMPLETED/ASSERTION_FAILED/PLAN_UPDATE) are NEVER `toolName` values.",
    "</tool_catalog>",
    "",
    "<cwv_targets>",
    "- LCP < 2500 ms",
    ...
    '- Cold-load: trace { command: "start", reload: true, autoStop: true }. INP profiling: trace { command: "start", reload: false, autoStop: false } → interact … → trace { command: "stop" }.',
    '- Drill every insight returned by `trace stop`: emit ACTION → trace { command: "analyze", insightSetId, insightName } for each (LCPBreakdown, CLSCulprits, RenderBlocking, NetworkDependencyTree, DocumentLatency, …).',
    "- One envelope per turn. No prose. No chained tool calls.",
    "</rules>",
    "",
    "<multi_modal_observations>",
    "State-changing ACTIONs refresh the screenshot in next <observation>; observe/trace calls do NOT refresh — re-issue a state-changing ACTION.",
    "Vision = grounding; snapshot text = UIDs (never click visual coords); on disagreement trust snapshot for targets and screenshot for post-action verification.",
    "</multi_modal_observations>",
  ].join("\n");
...
export const buildExecutionSystemPrompt = (browserMcpServerName?: string): string => {
  ...
  return [
    ...
    "</reflect_trigger>",
    "<completion_check>",
    "RUN_COMPLETED|passed only when every plan step has STEP_DONE and its expectedOutcome is verified against the latest <observed_state>. RUN_COMPLETED|failed only after a preceding ASSERTION_FAILED on the unsatisfied step with concrete evidence; to abort emit ASSERTION_FAILED|category=abort with abort_reason set, then RUN_COMPLETED|failed. Emit no further markers after RUN_COMPLETED.",
    "</completion_check>",
    `<tool_catalog server="${mcpName}">  // tool inventory + per-tool arg shapes are schema-enforced; do not invent names`,
    ...
    "- Emit STEP_START before each step; STEP_DONE or ASSERTION_FAILED before the next STEP_START.",
    ...
    "- Prefer click/fill/hover/select/wait_for over evaluate_script. Stay on the happy path; if blocked try one alternate action then abort.",
    "- One tool call per turn. Do not batch or chain speculatively.",
    "</rules>",
    "<multi_modal_observations>",
    "Each <observed_state> after a state-changing action includes a viewport screenshot; take_snapshot/list_console_messages/list_network_requests/perf-trace calls do NOT refresh.",
    "Vision = grounding; snapshot text = selectors (derive refs/UIDs, never click by visual coords); on disagreement trust snapshot for targets and screenshot for post-action verification.",
    "</multi_modal_observations>",
  ].join("\n");
};
