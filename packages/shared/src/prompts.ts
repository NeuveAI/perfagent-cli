...
export const buildLocalAgentSystemPrompt = (): string =>
  [
    ...
    "Emit PLAN_UPDATE when plan no longer matches observations. Common triggers: (a) tool keeps rejecting args after retry → action=replace with corrected step; (b) missing prerequisite found → action=insert; (c) step now redundant → action=remove; or analogous mismatch. payload is an AnalysisStep { id, title, instruction, expectedOutcome }. Cap: 5 per run; abort beyond.",
    "</plan_update_protocol>",
    "",
    "<reflect_trigger>",
    "After 1 ASSERTION_FAILED on the same stepId, the next observation contains a REFLECT directive — emit PLAN_UPDATE before retrying the step. Two ASSERTION_FAILEDs without an intervening PLAN_UPDATE force abort.",
    "</reflect_trigger>",
    "",
    "<failure_categories>",
...
export const buildExecutionSystemPrompt = (browserMcpServerName?: string): string => {
  ...
  return [
    ...
    "</protocol>",
    "<plan_update_protocol>",
    "Emit a PLAN_UPDATE marker BEFORE the next STEP_START when the plan no longer matches observations. Common triggers: (a) current step's tool keeps rejecting args after retry → action=replace with a corrected step; (b) missing prerequisite discovered → action=insert before current step; (c) current step now redundant → action=remove; or analogous mismatch. payload is a JSON AnalysisStep { id, title, instruction, expectedOutcome }. Markers:",
    ...
    "Cap: at most 5 PLAN_UPDATE markers per run. Beyond cap, abort.",
    "</plan_update_protocol>",
    "<reflect_trigger>",
    "After 1 ASSERTION_FAILED on the same step-id, the next turn's <observed_state> contains a REFLECT directive — emit PLAN_UPDATE before retrying. Two ASSERTION_FAILEDs without an intervening PLAN_UPDATE force abort.",
    "</reflect_trigger>",
