...
export const buildLocalAgentSystemPrompt = (): string =>
  [
    ...
    "args fields are exact (no `selector`/`ref`/`text` aliases): navigate{url|direction}; click/hover{uid}; fill{uid,value}; type{text}(focused-only); press_key{key}; wait_for{text:[]}. uid is from observe.snapshot.",
    "Envelope `_tag`s (THOUGHT/STEP_DONE/RUN_COMPLETED/ASSERTION_FAILED/PLAN_UPDATE) are NEVER `toolName` values.",
    "</tool_catalog>",
    "",
    "<cwv_targets>",
    "- LCP < 2500 ms",
