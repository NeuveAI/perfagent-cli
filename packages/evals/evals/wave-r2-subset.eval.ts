...
// Wave R2 calibration subset: the 5 shortest tasks (3 calibration + 2 trivial)
// used to verify the integrated R2 pipeline (native /api/chat + format grammar
// + ReAct prompts + AgentTurn-consuming tool-loop). Per PRD §R2 DoD:
// ≥80% schema-valid AgentTurn rate, ≥1 tool call/turn on at least 4 of 5 tasks,
// peakPromptTokens > 4096 (Probe D fix verification).
//   pnpm --filter @neuve/evals exec evalite run ./evals/wave-r2-subset.eval.ts
...
if (evalConfig.runner === "real") {
  ...
  registerRunnerSuite(runner, "wave-r2 subset real-runner");
} else if (evalConfig.runner === "gemma") {
  const runner: EvalRunner = makeGemmaRunner(evalConfig.gemmaOptions);
  registerRunnerSuite(runner, "wave-r2 subset gemma-runner");
} else {
  evalite<MockCaseInput, ExecutedTrace, EvalTask>("wave-r2 subset mock-runner", {
    data: () => buildMockCases(),
